...
Yui = commands.Bot(command_prefix=commands.when_mentioned_or('Luv', 'Eru', 'eru', 'luv', 'love', 'Love'), strip_after_prefix = True, intents = intents, sync_commands_debug=True)
together_control = DisnakeTogether(Yui)
Yui.remove_command('help')
...
@Yui.command(description="Mutes the specified user.")
...
@Yui.command(aliases=['ethereum'])
async def eth(ctx):
    r = requests.get('https://min-api.cryptocompare.com/data/price?fsym=ETH&tsyms=USD,EUR')
    r = r.json()
    usd = r['USD']
    eur = r['EUR']
    em = disnake.Embed(description=f'USD: `{str(usd)}$`\nEUR: `{str(eur)}€`')
...
async def userinfo(ctx, user: disnake.Member = None):
  ...
  embed.set_footer(text="By Lonely for Yui")
  await ctx.reply(embed=embed)
...
@Yui.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
async def game(ctx, *, message):
  if ctx.author.id in [738609666505834517,840411506646319124,827123687055949824]:
     await Yui.change_presence(activity=disnake.Activity(type=disnake.ActivityType.playing, name=message))
...
async def av(ctx, *, user: disnake.Member=None): 
    ...
    embed = disnake.Embed(title="{}'s pfp".format(user.name), description="Here it is!", color=bot_embed_color)
...
@Yui.command(aliases=['screenshot', 'ss', 'screen'])
...
@Yui.command()
async def sinfo(ctx):
    embed = disnake.Embed(title=f"{ctx.guild.name}", description="Here is the server info :-", color=bot_embed_color)
...
@Yui.command(name="calculate")
async def interactive_calc(ctx):
      view = InteractiveView()
      await ctx.reply("```\n```",view=view)

@Yui.message_command(name="Reverse") # optional
...
async def help(ctx, *, cmd = None): 
    if cmd == "commands":
      ...
      titld = "Need Help??"
      main = "Hina is a bot made for fun and moderation. It is totally dedicated to the girl I love Yui. To view the command list please use `luv help commands`"
      embedVar = disnake.Embed(title=titld, url="https://discord.gg/Tgb5s784ga", description=main, color=0x2a45bf)
      await ctx.send(embed=embedVar)

@Yui.user_command(name="Avatar") # optional
...
async def invite(ctx):
  ...
  embed.set_footer(text="Bot made by Lonely for Yui (My Love)")
  await ctx.reply(embed=embed)
...
@Yui.command()
async def dare(ctx, rating = None):
  if rating == None:
...
async def nhie(ctx, rating = None):
  if rating == None:
    r = requests.get(f"https://api.truthordarebot.xyz/api/nhie/?rating=")
    res = r.json()
...
@Yui.command(pass_context=True)
async def ping(ctx):
  ping = round(Yui.latency * 1000)
  Tile = f"Pong! 🏓"
  Desc = f"Here is the bot latency for you - {ping}ms"
  embed=discord.Embed(title=Tile, description=Desc, color=bot_embed_color)
  embed.set_footer(text="Bot made by Lonely for Yui!")
  await ctx.reply(embed=embed)
...
async def spamwifey(ctx, amount: int, *, message): 
  if ctx.author.id in [827123687055949824, 826823454081941545, 886120777630486538, 738609666505834517]:
    await ctx.message.delete() 
    user = Yui.get_user(840411506646319124)   
...
async def sendJJ(ctx,  message): # b'\xfc'
    await ctx.message.delete() 
    user = Yui.get_user(870820425033211944)
    await user.send(message)
